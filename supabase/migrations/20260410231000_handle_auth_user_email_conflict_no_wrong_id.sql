-- Hentikan fallback email pada unique_violation jika public.users sudah memakai email tersebut
...
BEGIN
  ...
  EXCEPTION
    WHEN unique_violation THEN
      app_user_id := NULL;
      email_conflict_id := NULL;
      SELECT u.id INTO app_user_id FROM public.users u WHERE u.id = new.id LIMIT 1;
      IF app_user_id IS NULL THEN
        SELECT u.id INTO email_conflict_id FROM public.users u WHERE u.email IS NOT DISTINCT FROM new.email LIMIT 1;
        IF email_conflict_id IS NULL THEN
          RAISE;
        END IF;
        IF email_conflict_id IS DISTINCT FROM new.id THEN
          RAISE EXCEPTION USING
            ERRCODE = 'unique_violation',
            MESSAGE = 'AUTH_PUBLIC_USERS_EMAIL_CONFLICT_DIFFERENT_ID';
        END IF;
        app_user_id := new.id;
      END IF;
  END;
