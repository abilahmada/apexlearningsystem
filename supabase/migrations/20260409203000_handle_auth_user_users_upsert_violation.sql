-- Tangani duplicate key di public.users (email ATAU id) agar signup tidak rollback auth.
-- Tanpa ini, INSERT ... ON CONFLICT (email) saja tetap bisa gagal jika bentrok di users_pkey.
...
BEGIN
  ...
  EXCEPTION
    WHEN unique_violation THEN
      ...
      WHERE u.id = new.id
      RETURNING u.id INTO app_user_id;
